...
export default function UploadDisbursementDonePage() {
  const hdrs = headers()
      ...
      <main className="max-w-md mx-auto px-6 py-20 text-center">
        <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
          ...
          <h1 className="serif font-black text-2xl text-slate-900 mb-2">
            {tFn('dsb.public.done.title', locale)}
          </h1>
          <p className="text-sm text-slate-600">{tFn('dsb.public.done.body', locale)}</p>
        </div>
      </main>
    </>
  )
