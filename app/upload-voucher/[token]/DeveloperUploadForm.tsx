...
import {
  createVoucherViaToken,
  requestVoucherUploadUrlsViaToken,
  registerVoucherUploadsViaToken,
} from './actions'
...
export function DeveloperUploadForm({
  locale,
  tokenRaw,
  suppliers,
  accounts,
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // 1) Create voucher.
      const create = await createVoucherViaToken({
        token_raw: tokenRaw,
      ...
      const urls = await requestVoucherUploadUrlsViaToken({
        token_raw: tokenRaw,
      ...
      // 4) Register the upload metadata + burn the token + fire n8n.
      const reg = await registerVoucherUploadsViaToken({
        token_raw: tokenRaw,
        voucher_id: voucherId,
      ...
      console.error('[DeveloperUploadForm] submit threw', err)
