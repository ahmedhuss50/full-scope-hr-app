...
export default async function DisbursementsAdminPage({
  ...
  // Count of imported units — feeds the owner-only "الوحدات والمشترون" card.
  // Cheap; unit rows are per-project so this is bounded by the physical
  // inventory.
  const { count: totalUnitsCount } = isOwner
    ? await svc
